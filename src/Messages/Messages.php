...
class Messages
{
    ...
    public static function getErrorMessage($errorID = null, $lang = Messages::LANG_IT, $givenValue = null, $showGivenValue = false)
    {
        ...
        $message = ((self::getMessage($errorID, $lang) ?:
            ...
            (self::ENABLE_DEFAULT_ERROR_FALLBACK ? self::getMessage("default.{$errorID}", $lang) : null)) ?: (self::ENABLE_DEFAULT_ERROR_FALLBACK ? self::getMessage('default.generic', $lang) : null)) ?:
            // fallback language
        ...
        if ($showGivenValue) {
            if (is_array($givenValue)) {
                $givenValue = json_encode($givenValue, JSON_THROW_ON_ERROR);
            }
