...
class Serializer
{
    ...
    public function encode(TypeInterface $type)
    {
        ...
        $linkedList = $type->getSetup()->getMiddlewaresAndRewind();
        ...
        while ($linkedList->hasNext()) {
            ...
            } elseif ($middleware instanceof Rule && $name === 'each') {
                $value = $middleware->getArgs()[0];
                if (is_array($value)) {
                    $value = V::group($value);
                }
                $serialized['rules'][$name][0] = $this->encode($value);
            } elseif ($middleware instanceof Rule && $name === 'or') {
                $setup = $middleware->getArgs()[0];
                foreach ($setup as $key => $value) {
                    if (is_array($value)) {
                        $value = V::group($value);
    ...
    public function decode(array $json): ?TypeInterface
    {
        $type = null;
